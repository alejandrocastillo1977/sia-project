...
MALLA_ISOV_VIRTUAL: Dict[str, Any] = {
    ...
    "nombre_malla": "Ingeniería de Software – UNIMINUTO Virtual",
    ...
    "plan": [
        {
            ...
            "cursos": [
                {
                    "codigo": "INFO 1030",
                    "nombre": "Habilidades Digitales para el Aprendizaje",
                    "creditos": 3,
                },
                {
                    "codigo": "LENG 1040",
                    "nombre": "Lectura y Escritura en el Contexto Digital",
                    "creditos": 3,
                },
        ...
        },
        {
            ...
            "cursos": [
                ...
                {
                    "codigo": "ISOF V023",
                    "nombre": "Estructuras de Datos y Análisis de Algoritmos",
                    "creditos": 3,
                },
                {
                    "codigo": "ISOF V033",
        ...
        },
        {
            ...
            "cursos": [
                ...
                {
                    "codigo": "ISOF V063",
                    "nombre": "Desarrollo de Software Orientado a la Web",
                    "creditos": 3,
                },
                {
                    "codigo": "ISOF V073",
                    "nombre": "Data Warehouse y Minería de Datos",
                    "creditos": 3,
                },
        ...
        },
        {
            ...
            "cursos": [
                {
                    "codigo": "UVFI D141",
                    "nombre": "Física Electromagnética",
                    "creditos": 3,
                },
                {
                    "codigo": "ISOF V123",
        ...
        },
        {
            ...
            "cursos": [
                ...
                {
                    "codigo": "PRAC 1010",
                    "nombre": "Práctica en Responsabilidad Social",
                    "creditos": 3,
                },
                {
                    "codigo": "ISOF V153",
                    "nombre": "Computación Bioinspirada",
                    "creditos": 3,
                },
                {
                    "codigo": "ISOF V163",
...
def validar_y_normalizar_malla(malla: Dict[str, Any]) -> Dict[str, Any]:
    """
    Valida y normaliza una estructura de malla curricular cargada (por ejemplo, desde JSON).

    Retorna un diccionario normalizado con las claves:
    - codigo_programa (mayúsculas, sin espacios extremos)
    - nombre_malla
    - programa (texto visible, por defecto igual a nombre_malla)
    - creditos_totales (int)
    - plan: lista de bloques con cuatrimestre (int) y cursos normalizados
    """
    if not isinstance(malla, dict):
        raise ValueError("La malla debe ser un objeto JSON de nivel raíz (tipo dict).")

    codigo_programa = str(malla.get("codigo_programa", "")).strip().upper()
    if not codigo_programa:
        raise ValueError("Falta el campo obligatorio 'codigo_programa'.")

    nombre_malla = str(malla.get("nombre_malla", "")).strip()
    if not nombre_malla:
        raise ValueError("Falta el campo obligatorio 'nombre_malla'.")

    creditos_totales_raw = malla.get("creditos_totales")
    try:
        creditos_totales = int(creditos_totales_raw)
    except (TypeError, ValueError):
        raise ValueError("El campo 'creditos_totales' debe ser un número entero.")

    plan_raw = malla.get("plan")
    if not isinstance(plan_raw, list) or not plan_raw:
        raise ValueError("La malla debe contener una lista 'plan' con al menos un cuatrimestre.")

    plan_normalizado: List[Dict[str, Any]] = []

    for bloque in plan_raw:
        if not isinstance(bloque, dict):
            raise ValueError("Cada elemento de 'plan' debe ser un objeto con 'cuatrimestre' y 'cursos'.")

        cuat_raw = bloque.get("cuatrimestre")
        try:
            cuatrimestre = int(cuat_raw)
        except (TypeError, ValueError):
            raise ValueError("Cada bloque de 'plan' debe tener un 'cuatrimestre' numérico.")

        cursos_raw = bloque.get("cursos")
        if not isinstance(cursos_raw, list) or not cursos_raw:
            raise ValueError(
                f"El cuatrimestre {cuatrimestre} debe contener una lista 'cursos' con al menos un curso."
            )

        cursos_norm: List[Dict[str, Any]] = []

        for curso in cursos_raw:
            if not isinstance(curso, dict):
                raise ValueError(
                    f"Cada curso del cuatrimestre {cuatrimestre} debe ser un objeto con 'codigo', 'nombre', 'creditos'."
                )

            cod_raw = curso.get("codigo")
            if isinstance(cod_raw, list):
                codigos_norm = [str(c).upper().strip() for c in cod_raw if str(c).strip()]
                if not codigos_norm:
                    raise ValueError(
                        f"En el cuatrimestre {cuatrimestre} hay un curso con lista 'codigo' vacía."
                    )
                codigo_norm: Any = codigos_norm
            else:
                codigo_str = str(cod_raw or "").upper().strip()
                if not codigo_str:
                    raise ValueError(
                        f"En el cuatrimestre {cuatrimestre} hay un curso sin 'codigo' válido."
                    )
                codigo_norm = codigo_str

            nombre = str(curso.get("nombre", "")).strip()
            if not nombre:
                raise ValueError(
                    f"En el cuatrimestre {cuatrimestre} hay un curso sin 'nombre' válido."
                )

            try:
                creditos = int(curso.get("creditos"))
            except (TypeError, ValueError):
                raise ValueError(
                    f"El curso '{nombre}' del cuatrimestre {cuatrimestre} tiene 'creditos' no numéricos."
                )
        ...
        plan_normalizado.append({"cuatrimestre": cuatrimestre, "cursos": cursos_norm})

    programa_visible = malla.get("programa") or nombre_malla

    return {
        "codigo_programa": codigo_programa,
        "nombre_malla": nombre_malla,
        "programa": programa_visible,
